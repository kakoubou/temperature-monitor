...
def set_screen_local(self, channel, mode, value, **kwargs):
    ...
    if mode == "brightness":
        ...
        data = value if isinstance(value, list) else self._prepare_static_file(value, self.orientation)
        ...
        mid_index = -1
        if memory_start == -1:
            if bucket_index == 1 or bucket_index == 0:
                for bI in range(1, 16):
                    self._delete_bucket(bI)
                memory_start = [0x41, 0x06]
            else:
                mid_index = bucket_index - 1
                #_delete_all_buckets(self, mid_index)
                self._delete_bucket(0)
                bucket_index = 0
